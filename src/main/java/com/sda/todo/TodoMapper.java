...
public class TodoMapper {
    ...
    public static  TodoModel map(String title,String description,String status,String priority,String data ){
        TodoModel model = new TodoModel();
        model.setName(title);
        model.setDescription(description);
//        model.setChecked("true".equals(status)); to tez jest poprawnie
//        model.setChecked(Boolean.valueOf(status));
        model.setChecked(new Boolean(status));
        model.setPriority(Integer.parseInt(priority));
        model.setDate(LocalDate.parse(data));
        return model;
    }
